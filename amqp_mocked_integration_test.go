...
func TestEventListenerSetup(t *testing.T) {
	...
	assert.Equal(t, QueueDeclaration{name: "events.topic.exchange.queue.svc", noWait: false, autoDelete: false, durable: true, args: amqp.Table{}}, channel.QueueDeclarations[0])
...
func TestServiceListener(t *testing.T) {
	...
	assert.Equal(t, 1, len(channel.QueueDeclarations))
	assert.Equal(t, QueueDeclaration{name: "svc.direct.exchange.request.queue", noWait: false, autoDelete: false, durable: true, args: amqp.Table{}}, channel.QueueDeclarations[0])
